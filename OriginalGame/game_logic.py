...
"""
from game_dimensions import Size_of_Board,Size_of_pieces,Num_pieces_per_size, Players, Size_of_Cell
...
class GobbletGameLogic :
    ...
    """
    def __init__(self):
        self.board = [[[] for _ in range(Size_of_Board)] for _ in range(Size_of_Board)]
        ...
        self.pieces_of_player = {
            Players.Red: {size: Num_pieces_per_size for size in range(len(Size_of_pieces))},
            Players.Blue: {size: Num_pieces_per_size for size in range(len(Size_of_pieces))}
        }
    ...
    def top_piece_present(self,row: int,col: int) :
        ...
        """
        if 0 <= row < Size_of_Board and 0 <= col < Size_of_Board and self.board[row][col] :
    ...
    def able_to_place_piece(self,row: int,col: int,size: int) -> bool:
        ...
        """
        if not (0 <= row < Size_of_Board and 0 <= col < Size_of_Board) :
            return False
    ...
    def conditions_win(self) -> bool:
        ...
        if self.line_match_check(diagonal_val_1) :
            return True
        for i in range(Size_of_Board) :
            diagonal_val_2.append(self.top_piece_present(i,Size_of_Board-i-1))
