...
import math
import pygame
from game_logic_final import Players,GobbletGameLogic
from game_dimensions_final import (WIDTH,HEIGHT,BLACK,WHITE,
                                   RED,BLUE,GREEN,SIZE_BOARD,SIZE_CELL,BOARD_X,BOARD_Y,SIZE_PIECES)
...
class GobbletGame :
    ...
    def draw_player_reserves(self) :
        ...
        """
        reserve_y = 70
        for size_idx, size in enumerate(SIZE_PIECES):
            for i in range(self.game_state.pieces_of_player[Players.RED][size_idx]):
                reserve_x = BOARD_X + size_idx * 150 + i * 65
                pygame.draw.circle(self.display, RED, (reserve_x, reserve_y), size)
                pygame.draw.circle(self.display, BLACK, (reserve_x, reserve_y), size, 2)
        reserve_y = HEIGHT-70
        for size_idx, size in enumerate(SIZE_PIECES):
            for i in range(self.game_state.pieces_of_player[Players.BLUE][size_idx]):
                reserve_x = BOARD_X + size_idx * 150 + i * 65
                pygame.draw.circle(self.display, BLUE, (reserve_x, reserve_y), size)
                pygame.draw.circle(self.display, BLACK, (reserve_x, reserve_y), size, 2)
    ...
    def game_information_display(self) :
        ...
        if self.game_state.present_player == Players.RED :
            player_data = "Current Player : RED"
        else :
            player_data = "Current Player : BLUE"
        ...
        if self.game_state.winner :
            ...
            if self.game_state.winner == Players.RED :
                winner_data = "RED Wins!!"
            else :
                winner_data = "BLUE Wins!!"
    ...
    def board_click_check(self,position) :
        ...
        """
        x,y = position
        if x < BOARD_X or x >= BOARD_X+SIZE_BOARD*SIZE_CELL :
            return None
        if y < BOARD_Y or y >= BOARD_Y+SIZE_BOARD*SIZE_CELL :
            return None
